...
from core.solvers.utils.par_tester_dev import HumParEvaluator, HumParEvaluator_bce_cls
...
class SolverMultiTaskDev(SolverDeter):
    ...
    def create_model(self):
        ...
        if self.config.dataset.type in ["COCOPosDatasetDev", "MultiPoseDatasetDev", 'MPIIPosDatasetDev']:
        ...
        elif self.config.dataset.type in ["ReIDDataset", 'AttrDataset']:
            self.config.decoder.kwargs.ignore_value = None
            self.config.decoder.kwargs.num_classes = 666 if self.config.dataset.type in ["ReIDDataset"] else 0 # compatablity fix, will be removed, not effective
        elif self.config.dataset.type in ["PedestrainDetectionDataset", 'CrowdHumanDetDataset']:
            self.config.decoder.kwargs.ignore_value = None
    ...
    def create_optimizer(self):
        ...
        memo = set()
        param_groups = []
        count = edict({'backbone': 0, 'decoder': 0, 'task_specific': 0, 'loss': 0, 'pe': 0})
        for module_name, module in self.model.named_modules():
            for module_param_name, value in module.named_parameters(recurse=False):
                ...
                if (
                    ...
                    hyperparams["weight_decay"] = 0.0
                if (value.task_specific or value.neck_specific) and self.config.get('task_specific_lr_scale', False):
        ...
        printlog(f">>> Backbone Params (including pe): {count.backbone / 1e6:.2f}M")
        printlog(f">>> Backbone PE Params: {count.pe / 1e6:.2f}M")
        printlog(f">>> Decoder Params: {count.decoder / 1e6:.2f}M")
        printlog(f">>> task_specific Params: {count.task_specific / 1e6:.2f}M")
        printlog(f">>> loss Params: {count.loss / 1e6:.2f}M")
        printlog(f">>> All Shared Params: {(count.backbone + count.decoder) / 1e6:.2f}M")
        printlog(f">>> All Params: {(count.task_specific + count.backbone + count.decoder) / 1e6:.2f}M")
...
class TesterMultiTaskDev(SolverMultiTaskDev):
    ...
    def run(self):
        if 'ParsingDataset' in self.config.dataset.type:
