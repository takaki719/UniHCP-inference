        ...
        for name, p in model.named_parameters():
            ...
            try:
                if p.task_specific:
                    ...
                    printlog(f'broadcasting backbone-specific param {name}\tgroup={share_backbone_group}')
                    broadcast(p, _get_global_rank(share_backbone_group, 0), group=share_backbone_group)
                elif p.neck_specific:
                    printlog(f'broadcasting neck-specific param {name}\tgroup={share_neck_group}')
                    broadcast(p, _get_global_rank(share_neck_group, 0), group=share_neck_group)
                elif p.decoder_specific:
                    printlog(f'broadcasting decoder-specific param {name}\tgroup={share_decoder_group}')
                    broadcast(p, _get_global_rank(share_decoder_group, 0), group=share_decoder_group)
                else:
                    printlog(f'broadcasting non-specific param {name}')
                    broadcast(p, 0)
            except Exception as e:
                printlog(f"****===>> p.task_specific: {p.task_specific}  "
                      f"p.backbone_specific: {p.backbone_specific}"
                      f" p.neck_specific: {p.neck_specific}"
                      f" p.decoder_specific: {p.decoder_specific}")
                printlog(f'error:{e}')
                raise RuntimeError(
                    'param {} does not have task_specific or backbone_specific or neck_specific or decoder_specific'.format(
                        name))
        ...
        for name, b in model.named_buffers():
            ...
            try:
                if b.task_specific:
                    ...
                    broadcast(b, _get_global_rank(share_backbone_group, 0), group=share_backbone_group)
                elif b.neck_specific:
                    printlog('broadcasting neck-specific buffer {}'.format(name))
                    broadcast(b, _get_global_rank(share_neck_group, 0), group=share_neck_group)
                elif b.decoder_specific:
                    printlog('broadcasting decoder-specific buffer {}'.format(name))
                    broadcast(b, _get_global_rank(share_decoder_group, 0), group=share_decoder_group)
                else:
                    broadcast(b, 0)
            except:
